...
export class AbonadosComponent implements OnInit, OnDestroy {
  ...
  cargarAbonados() {
    ...
    this.subcripciones.push( this._abonadoService.obtenerAbonados(this.desde, this.cantidad, this.columna, this.orden)
      .subscribe((resp: any) => {
      ...
      .subscribe(response => {
        ...
        switch (actionType) {
          case 'guardar':
            ...
            this.subcripciones.push( this._abonadoService.actualizarAbonado(abonado, abonado.id).subscribe((res) => {
                this.cargarAbonados();
              }, err => {
                console.log(err);
              })
  ...
  AgregarAbonado() {
    ...
    this.subcripciones.push( this.dialogRef.afterClosed()
      .subscribe((response: FormGroup) => {
        ...
        this.subcripciones.push(this._abonadoService.crearAbonado(abonado).subscribe(
          res => {
            this.cargarAbonados();
          }, err => {
            console.log(err);
  ...
  mostrarSWAEliminar(id: number) {
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          this.subcripciones.push( this._abonadoService.eliminarAbonado(id).subscribe((res) => {
              this.cargarAbonados();
            })
          )
        } else {
  ...
  buscarAbonado() {
    this.desde = 0;
    this.estaCargando = true;
    this.subcripciones.push( this._abonadoService.buscarAbonados(this.desde, this.cantidad, this.columna, this.orden, this.termino)
      .subscribe((resp: any) => {
        console.log(resp);
        this.total = resp.total;
        this.abonados = resp.abonados;
